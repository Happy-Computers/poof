...
func TestRelayPublishesAndServesWriterRanges(t *testing.T) {
	...
	publish := Stream{Name: "clip.mp4", Size: 6, State: "streaming"}
	...
	go func() {
		request, err := http.NewRequestWithContext(context.Background(), http.MethodGet, httpServer.URL+"/v1/writers/demo/next", nil)
		if err != nil {
			t.Error(err)
			return
		}
